...
public:
    TTuiDeviceImpl_(const char *resPath);
    ~TTuiDeviceImpl_();
...
//TTuiDevice proxy
TTuiDevice::TTuiDevice(const char *resPath) {
    impl_ = new TTuiDeviceImpl_(resPath);
}
...
void TTuiDevice::displayText(unsigned clr, unsigned line, unsigned col, const std::string &str) {
    impl_->displayText(clr, line, col, str);
}

void TTuiDevice::invertText(unsigned line) {
    impl_->invertText(line);
}

void TTuiDevice::clearText(unsigned clr, unsigned line) {
    impl_->clearText(clr, line);
}

void TTuiDevice::drawPNG(unsigned x, unsigned y, const char *filename) {
    impl_->drawPNG(x, y, filename);
}

void TTuiDevice::clearRect(unsigned clr, unsigned x, unsigned y, unsigned w, unsigned h) {
    impl_->clearRect(clr, x, y, w, h);
}

void TTuiDevice::drawText(unsigned clr, unsigned x, unsigned y, const std::string &str) {
...
TTuiDeviceImpl_::TTuiDeviceImpl_(const char *resPath) : eventQueue_(MAX_EVENTS), resPath_(resPath == nullptr ? "" : resPath) {
...
void TTuiDeviceImpl_::clearRect(unsigned clr, unsigned x, unsigned y, unsigned w, unsigned h) {
...
void TTuiDeviceImpl_::displayText(unsigned clr, unsigned line, unsigned col, const std::string &str) {
    unsigned x = col * 4;
...
void TTuiDeviceImpl_::invertText(unsigned line) {
    unsigned x = 0;
...
void TTuiDeviceImpl_::clearText(unsigned clr, unsigned line) {
    unsigned x = 0;
    unsigned y = (line * 10 + 10) + 1; // letters with drop
    cairo_set_source_rgb(cr_, colours[clr], colours[clr], colours[clr]);
...
void TTuiDeviceImpl_::drawPNG(unsigned x, unsigned y, const char *filename) {
    int img_w, img_h;
...
void TTuiDeviceImpl_::processGPIO() {
    ...
    int encdir[TTuiDeviceImpl_::MAX_ENCODERS] = {1, 1, 1, 1};
...
//                        fprintf(stderr, "encoder %d\t%d\t%lu\n", n, event.value, diff);
                        ...
                        if (diff > 100) { // filter out glitches
                            ...
                            TTuiEventMsg msg;
                            msg.type_ = TTuiEventMsg::N_ENCODER;
                            msg.id_ = n;
                            msg.value_ = event.value;
                            eventQueue_.enqueue(msg);
                        }
